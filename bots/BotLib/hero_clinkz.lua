...
function X.SkillsComplement()
    ...
    SearingArrowsDesire = X.ConsiderSearingArrows()
    if SearingArrowsDesire > 0
    then
        if not SearingArrows:GetAutoCastState() then
            SearingArrows:ToggleAutoCast()
        end
    else
        if SearingArrows:GetAutoCastState() and J.GetMP(bot) < 0.25 then
            SearingArrows:ToggleAutoCast()
        end
    end
...
-- Searing Arrows is now an autocast attack modifier (7.40 rework, replaces Tar Bomb)
function X.ConsiderSearingArrows()
    if not SearingArrows:IsTrained()
    then
        return BOT_ACTION_DESIRE_NONE
    end
    ...
    -- Always enable when fighting heroes
    if J.IsGoingOnSomeone(bot) or J.IsInTeamFight(bot, 1200)
    then
        if J.IsValidTarget(botTarget) and J.GetMP(bot) > 0.15
        then
            return BOT_ACTION_DESIRE_HIGH
        end
    end

    -- Enable when attacking Roshan/Tormentor
    if J.IsDoingRoshan(bot) or J.IsDoingTormentor(bot)
    then
        if J.IsAttacking(bot) and J.GetMP(bot) > 0.2
        then
            return BOT_ACTION_DESIRE_HIGH
        end
    end

    -- Enable when pushing buildings
    if J.IsPushing(bot) and J.IsAttacking(bot) and J.GetMP(bot) > 0.3
    then
        return BOT_ACTION_DESIRE_HIGH
    end

    -- Enable when farming with enough mana
    if J.IsFarming(bot) and J.IsAttacking(bot) and J.GetMP(bot) > 0.4
    then
        return BOT_ACTION_DESIRE_HIGH
    end

    -- Enable when harassing in lane
    if J.IsLaning(bot) and nEnemyHeroes ~= nil and #nEnemyHeroes >= 1
    and J.GetMP(bot) > 0.5
    then
        return BOT_ACTION_DESIRE_HIGH
    end
